...
* **`.env` file:** Create a file named `.env` in the root directory of your project (the same directory as `docker-compose.yml`) with the following format. These variables are used to configure the PostgreSQL database connection.

    ```bash
    # .env file
    POSTGRES_USER=postgres
    POSTGRES_PASSWORD=postgres
    POSTGRES_DBNAME=spond_analytics
    POSTGRES_PORT=5432
...
### 5. Current implementation's considerations and future Improvements
    
Current implementation's considerations and potential future improvement, please refer to the ```FUTURE_IMPROVEMENTS_CONSIDERATIONS.md``` file.
